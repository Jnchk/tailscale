...
import (
	...
	"github.com/Jnchk/tailscale/doctor"
	"github.com/Jnchk/tailscale/doctor/permissions"
	"github.com/Jnchk/tailscale/doctor/routetable"
	...
	"github.com/Jnchk/tailscale/ipn/policy"
	"github.com/Jnchk/tailscale/log/sockstatlog"
	...
	"github.com/Jnchk/tailscale/portlist"
	...
	"github.com/Jnchk/tailscale/wgengine/wgcfg/nmcfg"
)
	...
	if b.netMap != nil {
		if !hasCapability(b.netMap, tailcfg.CapabilitySSH) {
			if b.isDefaultServerLocked() {
				return errors.New("Unable to enable local Tailscale SSH server; not enabled on Tailnet. See https://github.com/Jnchk/tailscale/s/ssh")
			}
...
func (b *LocalBackend) sshOnButUnusableHealthCheckMessageLocked() (healthMessage string) {
	...
	return healthmsg.TailscaleSSHOnBut + "access controls don't allow anyone to access this device. Update your tailnet's ACLs at https://github.com/Jnchk/tailscale/s/ssh-policy"
}
...
func (b *LocalBackend) updateSELinuxHealthWarning() {
	if hostinfo.IsSELinuxEnforcing() {
		warnSSHSELinux.Set(errors.New("SELinux is enabled; Tailscale SSH may not work. See https://github.com/Jnchk/tailscale/s/ssh-selinux"))
	} else {
