...
type VersionInfo struct {
	...
	// for possible values.
	Long string
	// GitHash is the git hash of the github.com/Jnchk/tailscale Go module.
	GitHash string
...
func InfoFrom(dir string) (VersionInfo, error) {
	...
	if modPath == "github.com/Jnchk/tailscale" {
		// Invoked in the github.com/Jnchk/tailscale repo directly, just no further info to
		// collect.
	...
	// We seem to be in a repo that imports github.com/Jnchk/tailscale. Find the
	// github.com/Jnchk/tailscale repo and collect additional info from it.
...
// tailscaleModuleRef returns the git ref of the 'require github.com/Jnchk/tailscale' line
...
func tailscaleModuleRef(modBs []byte) (string, error) {
	...
	for _, req := range mod.Require {
		if req.Mod.Path != "github.com/Jnchk/tailscale" {
			continue
		}
	...
	return "", fmt.Errorf("no require github.com/Jnchk/tailscale line in go.mod")
}
...
func makeMSIProductCodes(v verInfo, track string) map[string]string {
	urlBase := fmt.Sprintf("https://pkgs.github.com/Jnchk/tailscale/%s/tailscale-setup-%d.%d.%d-", track, v.major, v.minor, v.patch)
...
// unknownPatchVersion is the patch version used when the github.com/Jnchk/tailscale package
...
func infoFromCache(ref string, runner dirRunner) (verInfo, error) {
	...
	if _, err := os.Stat(tailscaleCache); err != nil {
		if !runner.ok("git", "clone", "https://github.com/tailscale/tailscale", tailscaleCache) {
			return verInfo{}, fmt.Errorf("cloning github.com/Jnchk/tailscale repo failed")
		}
	}
